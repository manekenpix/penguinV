...
namespace
{
    ...
    const std::string programCode = R"(
        ...
        __kernel void isEqualOpenCL( __global const uchar * in, uint offsetIn, uint rowSizeIn, __global uchar * out, uint offsetOut, uint rowSizeOut, uint width, uint height, volatile __global uint * emptyCount )
        {
            const size_t x = get_global_id(0);
            const size_t y = get_global_id(1);
